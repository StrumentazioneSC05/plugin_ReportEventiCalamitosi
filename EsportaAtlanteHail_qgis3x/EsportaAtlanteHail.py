...
global qgis_version
try:
    from qgis.core import * #versione qgis 3.x
except ImportError:
...
class getPostsThread(QThread):
    #riporto le azioni QObject.connect da PyQt4 a PyQt5
...
class EsportaAtlanteHail:
    ...
    def get_Atlas_filter(self):
        ...
        #nuova versione QGis 3.x prove:
        project = QgsProject.instance()
        projectLayoutManager = project.layoutManager()
        myComposition = projectLayoutManager.layouts()[0]
        myAtlas = myComposition.atlas
    ...
    def inizializza_layer(self):
        ...
        #questo metodo pare non funzionare piu' su QGis 3.x. Dunque:
        layers_caricati = QgsProject.instance().mapLayers()
        #Utils.logMessage("Primo layer: " + layers_caricati[0].name()) #con la versione QGis 3.x questo e' un dict dunque questa chiamata non funziona piu'
        #Altro metodo:
        ...
        for layer in layers_caricati.values():
    ...
    def connetto_pulsanti(self):
        ...
        #QObject.connect(self.dlg.combo_layer, SIGNAL("currentIndexChanged(const QString&)"), self.updateFromSelection_layers)
        #QObject.connect(self.dlg.combo_fields, SIGNAL("currentIndexChanged(const QString&)"), self.get_field_type)
        self.dlg.combo_layer.currentIndexChanged.connect(self.updateFromSelection_layers)
        self.dlg.combo_fields.currentIndexChanged.connect(self.get_field_type)
        #self.combo_layer.accepted.connect(self.updateFromSelection_layers)
